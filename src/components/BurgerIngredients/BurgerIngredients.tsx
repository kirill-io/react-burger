...
export const BurgerIngredients = () => {
  ...
  const buns = useMemo(() => {
    return ingredients.filter((item: IIngredientData) => item.type === "bun");
  }, [ingredients]);
  const mains = useMemo(() => {
    return ingredients.filter((item: IIngredientData) => item.type === "main");
  }, [ingredients]);
  const sauces = useMemo(() => {
    return ingredients.filter((item: IIngredientData) => item.type === "sauce");
  }, [ingredients]);

  const bunRef = useRef<HTMLHeadingElement | any>(null);
  const mainsRef = useRef<HTMLHeadingElement | any>(null);
  const sauceRef = useRef<HTMLHeadingElement | any>(null);
